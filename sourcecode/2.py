...
with open('sourcecode/top_3.txt',"w", encoding='utf-8') as f:
    for i in col:
        tmp = []
...
data_frame.to_csv('sourcecode/results2.csv')
...
for x in columns:
    ...
    plt.savefig(f'sourcecode/image/all_player/statistic_of_{x.replace('/', '_per_')}.png',dpi=300)
    plt.close()
...
for team in teams:
    ...
    for x in columns:
        ...
        plt.ylabel("Frequency")
        plt.savefig(f"sourcecode/image/eachTeam/statistic_of_{x.replace('/', 'per')}_team_{team}.png",dpi = 300)
        plt.close()
